...
**Last updated:** 2026-06-19 WIB
**Current state:** Phase 9 (feedback iteration) complete. All gates green: backend 95 unit + 54 e2e + build + lint; frontend lint + 47 tests + build. New endpoints shipped: `POST /auth/register`, `GET /cases/:id/invites`, `POST /cases/:id/accept`, `GET /cases/:id/recommendations`, `DELETE /cases/:caseId/documents/:id`. Schema migration `20260619083652_feedback_iteration` applied. Seed expanded to 3 parents / 10 tutors / 12 cases / 14 invites / 9 documents. Phase 8 deploy still pending.
...
| 9 | Feedback Iteration | ✅ code + unit + e2e + frontend tests + build all green |
...
| 2026-06-19 | 9 | Feedback iteration shipped. Schema migration `20260619083652_feedback_iteration` added `Case.description`, `Case.matchedTutorId`, `Document.deletedAt`. New backend endpoints: `POST /auth/register` (public signup, rate-limited, bcrypt, TUTOR bootstraps profile), `GET /cases/:id/invites` (owner-only, lists invited tutors with name+qualifications), `POST /cases/:id/accept` (owner-only, sets MATCHED+matchedTutorId, 409 on second tutor), `GET /cases/:id/recommendations` (owner-only AI-mock heuristic, deterministic, no external LLM), `DELETE /cases/:caseId/documents/:id` (uploader-only soft-delete). Profile-doc delete also converted to soft. `budgetPerHour` `@Min(1)`. Frontend: `/` welcome/landing page, `/signup` (RHF+zod, role select), invite-tutor component (name search→invite, invited list, accept/revoke, matched badge), recommendation-panel (loading/empty/error), case-documents (grouped by uploader, per-doc delete, multi/drag-drop), parent nav refocused cases-centric, tutor "Cases" nav labelled "Invitations", new hooks (useCaseInvites, useRecommendations, useAcceptTutor, useDeleteCaseDocument, useDeleteOwnDocument, register mutation). Seed expanded: 3 parents, 10 tutors (subject spread), 12 cases (all statuses, ≥2 MATCHED with matchedTutorId, one budgetPerHour=1), 14 invites, 9 documents (one soft-deleted). Gates: backend 95 unit + 54 e2e + build + lint green; frontend lint + 47 tests + build green. | Phase 8 deploy still pending. |
